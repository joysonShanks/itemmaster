...
public class ResourceConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		...
		//http.csrf().disable().anonymous().disable().authorizeRequests()
				...
				.antMatchers("/itemmaster/product-basic").hasAnyAuthority("ADMIN", "USER")
				.antMatchers("/itemmaster/product/admin/**").hasAnyAuthority("ADMIN")
	...
	private static class OAuthRequestedMatcher implements RequestMatcher {
		public boolean matches(HttpServletRequest request) {
			...
			if (path.length() >= 12) {
				...
				boolean isApi = path.equals("/itemmaster/");
				return isApi;
			} else {
				return false;
			}
